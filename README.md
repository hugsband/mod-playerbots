> ## Bear Kingdom EQ fork — what's custom here
>
> This is a fork of [mod-playerbots/mod-playerbots](https://github.com/mod-playerbots/mod-playerbots) (GPLv2) that runs on the Bear Kingdom EverQuest-in-WoW server. Everything in this section is **our custom work on the `eqwow` branch, on top of upstream** — if you're comparing against the original project, this list is why the diff looks the way it does:
>
> - **AI bot chat (ChatBrain).** Bots answer whispers and chat in say/party/guild/world with lines generated by a local LLM on our own hardware. Most of that system lives in our mod-everquest module and an external orchestrator; this fork's piece is suppressing the old canned whisper replies when ChatBrain is on, so you get one real answer instead of "I don't understand."
